...
.option('--testing', 'Whether or not to test the mapper output', false)
...
if(!cmd.testing) {
    ...
    myRequest.then(function(files) {
        console.log("post myRequest");
        ...
        return Promise.all(files.map(function(file, index){
            ...
            // console.log(contentfulApi + '/entries');
            // console.log({
            //     'Authorization': 'Bearer ' + cmd.token,
            //     'X-Contentful-Content-Type': cmd.contentType
            // });
            // console.log(JSON.stringify({ fields: data.fields,  }));
            var apiOptions = {
                method: 'POST',
                // json: true,
